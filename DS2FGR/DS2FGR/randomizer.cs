	...
	public class Randomizer
	{
		...
		void write_string_to_file(String str, String file_path)
		{
			try
			{
				...
				Console.WriteLine($"[SUCCESS] {Path.GetFullPath(file_path)} created.");
			}
			catch (Exception ex)
			{
		...
		List<List<WarpNode>> segments_flat;
		List<WarpInfo> warp_infos;
		List<Warp> warps;
		int fix_warps_count;
		String mod_folder;
        ...
        public async Task randomize(int seed)
		{
			...
			while (true)
			{
				...
				if (cnt > 100000)
				{
					Console.WriteLine("[INFO] Failed to find a path reiterating.");
					await randomize(rand.Next());
				}
			}
...
Hint: Turn word wrap off from [View > Word Wrap]
INFO: suffix Front and Back below specify either side of the fog gate
      Front usually means the way you usually encounter the fog gate while traversing the game normally
	  Back  is the opposite side of Front.

");
			write_string_to_file(sb.ToString(), Path.Join(log_dir, $"log_{DateTime.UtcNow:yyyy-MM-dd-HH-mm-ss}_{seed}.txt"));
			...
			{
				Console.Write($"[INFO] saving map {map_name}...");
